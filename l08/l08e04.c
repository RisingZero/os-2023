...
int main(int argc, char *argv[]) {
    ...
    for (task = A; task < TASKS_NUM; task++) {
        ...
        switch (task) {
            case A:
                ...
                task_data[task]->task_dependancies[0] = &semaphores[I];
    ...
    // Start thread A for the first time
    sem_post(&semaphores[I]);

    // Wait until SIGKILL to process, threads never end
...
void* thread_task(void* args) {
    ...
    int i;
    fprintf(stdout, "Thread %c started\n", OFFSET + data->task_id);
    while (1) {
